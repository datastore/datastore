...
def compare(a, b):
    b = list(b)
    ''' compare two list without ordering'''
...
class TestSymlinkDatastore(TestDatastore):
    ...
    def test_symlink_internals(self):
        ...
        assert(compare(dds_query(), [1, lva]))  # assumed ordering unimportant
        ...
        assert (compare(dds_query(), [2, lva]))
        ...
        assert (compare(dds_query(), [3, lva]))
        ...
        assert (compare(dds_query(), [3, lva, lva]))
        ...
        self.assertEqual(set(dds_query()), {3, lva, lvb, lvc})
...
class TestDatastoreCollection(TestDatastore):
    ...
    def test_sharded(self, numelems=1000):
        ...
        def check_for(key, value, sharded, shard=None):
        ...
        for value in range(0, numelems):
            ...
            check_for(key, value, sharded)
            shard.put(key, value)
            check_for(key, value, sharded, shard)
        ...
        for value in range(0, numelems):
            ...
            check_for(key, value, sharded, incorrect_shard)
        self.assertEqual(sumlens(stores), numelems)
        ...
        for value in range(0, numelems):
            ...
            check_for(key, value, sharded, correct_shard)
        self.assertEqual(sumlens(stores), numelems)
