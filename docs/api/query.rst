...
native form (e.g. into SQL for MySQL or a MongoDB query). However, on the
...
(e.g. using a :ref:class:`FileSystemDatastore` and not worry about a database
at all).
When it comes time to switch the tool for performance, updating the
...
tl;dr: queries are supported across datastores. They are very cheap on top of
relational databases, and very expensive otherwise. Pick the right tool for the
job!
